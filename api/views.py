...
def sources(request):
    ...
    try:
        if userId == "":
            ...
            cursor.execute("SELECT DISTINCT source, COUNT(source) AS nb"
                           "FROM api_tweet"
                           "WHERE user_id_id ='" + str(userId) + "'"
...
def user(request):
    ...
    try:
        if userId == "":
            cursor.execute(
                ...
                "name, description, followers_count,"
                "verified, profile_image_url FROM api_user")
        else:
            cursor.execute(
                "SELECT id, screen_name, name, description,"
                "followers_count, verified, profile_image_url "
                "FROM api_user "
                "WHERE id ='" + str(userId) + "'")
    except BaseException as e:  # No data in DB
        print("API - userInfo()\nError : ", e)
    ...
    for user in res:
        users.append(
            {"id": user[0], "screen_name": user[1], "name": user[2], "description": user[3], "followers_count": user[4],
             "verified": user[5], "profile_image_url": user[6].replace('_normal.jpg', '.jpg')})
...
def nbTweets(request):
    ...
    try:
        cursor.execute(
            "SELECT u.name, COUNT(t.id) AS nbTweets "
            "FROM api_tweet t, api_user u "
            "WHERE u.id = t.user_id_id "
            "GROUP BY u.name "
            "ORDER BY nbTweets DESC")
    except BaseException as e:  # No data in DB
        print("API - nbTweets()\nError : ", e)
...
def hours(request):
    ...
    try:
        if userId == "":
            ...
            cursor.execute("SELECT DISTINCT created_at AS timePosted FROM api_tweet WHERE user_id_id ='" + str(
                userId) + "'ORDER BY timePosted")
    except BaseException as e:  # No data in DB
        print("API - hours()\nError : ", e)
...
def wordCount(request):
    ...
    words = to_json_for_graph(count_words(words))
...
def lemmeCount(request):
    ...
    lemmes = to_json_for_graph(count_words(lemmes))
...
def netTweets(request):
    ...
    try:
        ...
        cursor = connection.cursor()
        cursor.execute(
            "SELECT u.screen_name, COUNT(t.id)"
            "AS nbTweets FROM api_tweet t, api_user u "
            "WHERE u.id = t.user_id_id "
    ...
    for user in users:
        network["nodes"].append(
            {"id": nodeId, "label": user["name"], "screen_name": user["screen_name"], "size": 50, "color": "#ff9800"})
    ...
    for semanticLine in semanticField_db:
        if semanticLine["baseWord"] not in appendedThemes:
            network["nodes"].append(
                {"id": nodeId, "label": semanticLine["baseWord"], "weight": 0, "size": 50, "color": "#2196f3"})
            network["nodes"].append(
                {"id": nodeId + 1, "label": semanticLine["baseWord"], "size": 10, "color": "#607d8b"})
            network["edges"].append(
                {"id": edgeId, "source": nodeId, "target": nodeId + 1, "weight": 50, "color": "rgba(96,125,139,0.05)"})
    ...
    for semanticLine in semanticField_db:
        ...
        if not all(score == 0 for score in uScores.values()):
            if semanticLine["word"] not in appendedNodes:
                ...
                network["edges"].append(
                    {"id": edgeId, "source": appendedThemes[semanticLine["baseWord"]], "target": nodeId, "weight": 50,
                     "color": "rgba(96,125,139,0.05)"})
                appendedNodes[semanticLine["word"]] = nodeId
                nodeId += 1
            else:
                network["edges"].append({"id": edgeId, "source": appendedThemes[semanticLine["baseWord"]],
                                         "target": appendedNodes[semanticLine["word"]], "weight": 50,
                                         "color": "rgba(96,125,139,0.05)"})
            edgeId += 1
            ...
            for screen_name, score in uScores.items():
                if score != 0:
                    ...
                    network["edges"].append({"id": edgeId, "source": appendedNodes[screen_name],
                                             "target": appendedNodes[semanticLine["word"]], "weight": computedWeigth,
                                             "color": "rgba(255,152,0,0.01)"})
                    edgeId += 1
...
def getData(request):
    ...
    global screen_name_to_extract
    global semanticWords

    # SAVING USERS
    for screen_name in screen_name_to_extract:
        print("GET User info : " + screen_name)
        userInfo = return_user(screen_name, toClean=True)
    ...
    for screen_name in screen_name_to_extract:
        ...
        try:  # We try to get the id of the user's last tweet
            ...
            return JsonResponse({"res": {"success": success, "error": e}}, status=500)

        # The important request here : returns the new tweets from the Twitter API
        tweets = return_tweets_multiple(screen_name, lastId)
        ...
        tweets = tokenize_and_lemmatize_tweets(tweets)

        # Saving tweets in database
        userFrom = User.objects.get(screen_name=screen_name)
        nbTweetsSaved = 1;  # counter for printing
        for t in tweets:
            print("SAVING {1} TWEETS FROM {0} ; {2:0.2f} %".format(screen_name, lenghtTweets,
                                                                   nbTweetsSaved / lenghtTweets * 100))
            success = save_tweet(t, userFrom) and success
            nbTweetsSaved += 1;
    ...
    for baseWord in semanticWords:
        semanticFieldTable = get_semantic_field(baseWord)
