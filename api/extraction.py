...
def oauth_request(url, credentials, http_method="GET", post_body="", http_headers=None):
...
def return_tweets_batch(screen_name, count=False, max_id=False, since_id=False):
    ...
    # Request
    res = oauth_request(url, credentials)
...
def return_tweets_multiple(screen_name, lastId=0):
    ...
    if not screen_name in screen_name_to_extract:
        return []

    tweets = []  # List of the nbTweet tweets
    currentId = return_tweets_batch(screen_name, 1)[0]["id"]  # id of the user's last tweet
    ...
    if lastId == 0:  # We only want to get ALL the tweets ; it supposes there are no tweets in the DB
        ...
        while lastId != currentId and iterNum < maxIter:
            tweets += return_tweets_batch(screen_name, batchSize, currentId, False)
            # Updating the ids
            lastId = currentId
        ...
        catch = return_tweets_batch(screen_name, batchSize, currentId, lastId)
        while catch:
            tweets += catch
            # Updating the id
            currentId = tweets[-1]["id"] - 1
...
def return_user(screen_name, toClean=True):
    ...
    # Request
    res = oauth_request(baseURL + 'users/show.json?screen_name=' + screen_name, credentials)
...
def clean_user(user):
    ...
    global useful_fields_user
    global stringFields

    fieldsToDelete = [k for k in user.keys() if k not in useful_fields_user]
    for key in fieldsToDelete:
        del user[key]
...
# Credentials
consumerKey = get_env_value('CONSUMER_KEY')
consumerSecret = get_env_value('CONSUMER_SECRET')
...
useful_fields_user = ["id", "name", "screen_name", "created_at",
                      "contributors_enabled", "verified", "description", "followers_count", "profile_image_url"]

screen_name_to_extract = ["n_arthaud",
                          "UPR_Asselineau",
                          "JCheminade",
                          "dupontaignan",
                          "FrancoisFillon",
...
def test_batch(screen_name, count=False, max_id=False, since_id=False):
    res = return_tweets_batch(screen_name, count, max_id, since_id)
    for t in res:
        print(t["id"], t["text"])
...
def test_multiple(screen_name, since_id=False):
    res = return_tweets_multiple(screen_name, since_id)
    for t in res:
        print(t["id"], t["text"])
...
def test_profile(screen_name, toClean=True):
    user = return_user(screen_name, credentials, toClean)
